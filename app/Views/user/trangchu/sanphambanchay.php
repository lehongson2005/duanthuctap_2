<?php 
// CSS cho component SẢN PHẨM BÁN CHẠY
...
<style>
   ...
   .product-image-container {
    position: relative;
    width: 100%;
    aspect-ratio: 1 / 1;      /* Vuông tuyệt đối – hiện đại */
...
.product-hover-img {
    ...
    width: 100%;
    height: 100%;
    object-fit: cover;        /* Vừa khung – không to không nhỏ */
    transition: opacity 0.3s ease, transform 0.3s ease;
}

.product-card:hover .product-hover-img {
    opacity: 1;
    transform: scale(1.05);   /* Zoom nhẹ cho đẹp */
}
...
.product-card:hover .product-main-img {
    ...
    .product-options-bestseller .btn {
        border-radius: 5px !important;
        padding: 8px 15px;
        font-weight: bold;
    }
    .product-options-bestseller .btn-check:checked + .btn {
...
<section id="bestseller-list-section" class="mt-4">
    <h3 class="fw-bold mb-3">SẢN PHẨM BÁN CHẠY</h3>
    
    <div class="nnp-product-slider-wrapper">
        
        <button class="nnp-slider-nav-btn nnp-prev-btn d-none d-lg-flex" onclick="scrollSlider('bestseller-list-slider', 'prev')">
            <i class="fas fa-chevron-left"></i>
        </button>

        <div class="nnp-product-slider" id="bestseller-list-slider">
            <?php if ($sanphambanchay_products && $sanphambanchay_products->num_rows > 0): ?>
                <?php while ($product = $sanphambanchay_products->fetch_assoc()): ?>
                    <div class="col">
        ...
        <button class="nnp-slider-nav-btn nnp-next-btn d-none d-lg-flex" onclick="scrollSlider('bestseller-list-slider', 'next')">
