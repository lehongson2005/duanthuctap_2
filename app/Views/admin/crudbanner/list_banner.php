...
$total_records = $bannerModel->getTotal($keyword, $status);
$total_pages = ceil($total_records / $limit);

// Get records for the current page
$items = $bannerModel->searchAndFilter($keyword, '', $status, $limit, $offset);
?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="fw-bold text-dark mb-0"><?php echo $page_title; ?></h2>
        <p class="text-muted small">Danh sách toàn bộ banner trong hệ thống</p>
    </div>
    <a href="add_banner.php" class="btn btn-primary shadow-sm">
...
<div class="card shadow-sm border-0 rounded-4 mb-4">
    <div class="card-body">
        <form method="GET" action="">
            <div class="row g-3">
                <div class="col-md-6">
                    <input type="text" name="keyword" class="form-control" placeholder="Tìm theo tiêu đề, vị trí..." value="<?php echo htmlspecialchars($keyword); ?>">
                </div>
                <div class="col-md-4">
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    <?php if($items->num_rows > 0): ?>
                        ...
                        <tr>
                            <td class="ps-4">
                                <img src="<?php echo BASE_URL . '/' . htmlspecialchars($row['image']); ?>" alt="<?php echo htmlspecialchars($row['title']); ?>" class="me-3 rounded-3" style="width: 120px; height: 60px; object-fit: cover;">
                            </td>
                            <td>
                                <div class="fw-bold text-dark"><?php echo htmlspecialchars($row['title']); ?></div>
                                <a href="<?php echo htmlspecialchars($row['link']); ?>" target="_blank" class="small text-muted"><?php echo htmlspecialchars($row['link']); ?></a>
                            </td>
                            <td><?php echo htmlspecialchars($row['position']); ?></td>
                            <td>
                                <?php if($row['start_date'] && $row['end_date']): ?>
                                    <small>Từ: <?php echo date('d/m/Y', strtotime($row['start_date'])); ?></small><br>
                                    <small>Đến: <?php echo date('d/m/Y', strtotime($row['end_date'])); ?></small>
                                <?php else: ?>
                                    <small>Luôn có hiệu lực</small>
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="pe-4 text-end">
                                <a href="edit_banner.php?id=<?php echo $row['id']; ?>" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a>
                                <a href="delete_banner.php?id=<?php echo $row['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Bạn có chắc muốn xóa banner này?')"><i class="fas fa-trash-alt"></i></a>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                    <tr>
                        <td colspan="7" class="text-center py-5 text-muted">Không tìm thấy banner nào.</td>
                    </tr>
                    <?php endif; ?>
                </tbody>
            </table>
