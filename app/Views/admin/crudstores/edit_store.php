...
// Fetch store data for GET request (or if POST failed)
...
// Use POST values if submitted, otherwise use existing store data
$selected_province_id = $_POST['province_id'] ?? $store['province_id'];
$selected_ward_id = $_POST['ward_id'] ?? $store['ward_id'];
$wards_for_dropdown = null;

// If a province was selected, get its wards
if (!empty($selected_province_id)) {
    $wards_for_dropdown = $wardModel->getByProvinceId($selected_province_id);
}

// Handle POST request
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['province_id_changed'])) { // Avoid re-processing on province change form submit
    ...
    if (empty($name) || empty($address) || $province_id <= 0 || $ward_id <= 0) {
        ...
        if ($storeModel->update($id, $name, $address, $ward_id, $phone_number, $map_link, $status, $latitude, $longitude)) {
            $_SESSION['success_message'] = "Cập nhật Cửa hàng thành công.";
            header("Location: list_stores.php");
            exit;
        } else {
            $_SESSION['error_message'] = "Cập nhật Cửa hàng thất bại. Vui lòng thử lại.";
        }
    }
}
?>
...
<div class="container-fluid">
    <h1 class="h3 mb-2 text-gray-800">Sửa Cửa hàng</h1>
    <p class="mb-4">Chỉnh sửa thông tin cho Cửa hàng có ID: <?= htmlspecialchars($store['id']) ?>.</p>

    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Form sửa Cửa hàng</h6>
        </div>
        <div class="card-body">
            <form action="edit_store.php?id=<?= $id ?>" method="POST">
                <div class="form-group">
                    <label for="name">Tên Cửa hàng:</label>
                    <input type="text" class="form-control" id="name" name="name" required value="<?= htmlspecialchars($_POST['name'] ?? $store['name']) ?>">
                </div>
                <div class="form-group">
                    <label for="address">Địa chỉ (Số nhà, tên đường):</label>
                    <input type="text" class="form-control" id="address" name="address" required value="<?= htmlspecialchars($_POST['address'] ?? $store['address']) ?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="ward_id" name="ward_id" required>
                        ...
                        <?php if ($wards_for_dropdown) : ?>
                            <?php $wards_for_dropdown->data_seek(0); // Reset result pointer ?>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="phone_number" name="phone_number" value="<?= htmlspecialchars($_POST['phone_number'] ?? $store['phone_number']) ?>">
                </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="latitude" name="latitude" value="<?= htmlspecialchars($_POST['latitude'] ?? $store['latitude']) ?>" placeholder="Ví dụ: 10.7769">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="longitude">Kinh độ (Longitude):</label>
                            <input type="text" class="form-control" id="longitude" name="longitude" value="<?= htmlspecialchars($_POST['longitude'] ?? $store['longitude']) ?>" placeholder="Ví dụ: 106.7009">
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    <label for="map_link">Link Google Maps:</label>
                    <input type="url" class="form-control" id="map_link" name="map_link" value="<?= htmlspecialchars($_POST['map_link'] ?? $store['map_link']) ?>">
                    <small class="form-text text-muted">Ví dụ: https://goo.gl/maps/example</small>
                </div>
                <div class="form-group form-check">
                    <input type="checkbox" class="form-check-input" id="status" name="status" value="1" <?= ((isset($_POST['status']) && $_POST['status'] == '1') || $store['status']) ? 'checked' : '' ?>>
